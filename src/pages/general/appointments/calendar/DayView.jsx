...
export default function DayView({
  ...
  // ✅ keep currentDate in sync with parent startDate
  useEffect(() => {
    setCurrentDate(startDate);
  }, [startDate]);

  // Convert "04:00 PM" -> "16:00:00"
  ...
  const format12H = (timeStr) => {
    return dayjs(`1970-01-01T${timeStr}`).format("hh:mm A");
  };
  ...
  // Fetch events when date or Doctor changes
  useEffect(() => {
    async function fetchEvents() {
      try {
        const [internalRes, webRes] = await Promise.allSettled([
          AxiosInstance.get(`/appointments/date?date=${currentDate}`),
        ...
        // Filter Web appointments by date
        webAppts = webAppts.filter((appt) => {
          if (!appt.appointmentDate) return false;
          return (
            dayjs(appt.appointmentDate).format("YYYY-MM-DD") === currentDate
          );
        });
        ...
        const syncedWebIds = new Set(eventsArray.map(l => l.webAppointmentId).filter(Boolean));
        webAppts = webAppts.filter(w => !syncedWebIds.has(w._id));

        eventsArray = [...eventsArray, ...webAppts];
        ...
        const dayPilotEvents = eventsArray.map((appt) => {
          const datePart = appt.date.split("T")[0];

          let start24, end24;

          if (appt.isWebAppointment && appt.selectedSlot) {
            const timeParts = appt.selectedSlot.split("-");
            start24 = convertTo24H(timeParts[0].trim());
            end24 = timeParts[1]
              ? convertTo24H(timeParts[1].trim())
              : undefined;
          } else {
            start24 = convertTo24H(appt.startTime);
          }

          const start12 = format12H(start24);

          if (!end24) {
            // small buffer for event end
            ...
            end24 = `${endHours.toString().padStart(2, "0")}:${endMinutes
              .toString()
              .padStart(2, "0")}:00`;
          }
          ...
          return {
            id: appt._id,
            text: `${appt.isWebAppointment ? "🌐 " : ""}${start12} | ${appt.patientName}`,
            start: `${datePart}T${start24}`,
            end: `${datePart}T${end24}`,
            backColor: colorMap[usedStatus] || "#94a3b8",
            apptData: appt,
            cssClass: `${appt.status === "Cancelled" ? "line-through grayscale opacity-50" : "shadow-md hover:shadow-lg transition-all"}`,
          };
        });
    ...
    fetchEvents();
  }, [currentDate, selectedGroup, stateChange, refreshTrigger]);
  ...
  const handleEventClick = (args) => {
    setSelectedAppt(args.e.data.apptData);
                ...
                <div className="flex items-center gap-3 text-slate-600 ">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="font-semibold">
                      ...
                      {selectedAppt.endTime && selectedAppt.endTime !== "TBD"
                        ? ` - ${format12H(convertTo24H(selectedAppt.endTime))}`
                        : ""}
                    </span>
                  </div>
                </div>
