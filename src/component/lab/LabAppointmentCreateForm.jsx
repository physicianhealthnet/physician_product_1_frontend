...
function LabAppointmentCreateForm() {
  ...
  const fetchActiveLabs = async () => {
    try {
      const res = await AxiosInstance.post('/lab-prescription/by-status', { 
      ...
      console.error("Failed to fetch live lab data", err);
    }
  };
  ...
  const handleCreatePrescription = async (e) => {
    ...
    try {
      const appointmentDateTime = dayjs(`${formData.date}T${formData.time}`).toISOString();
      const payload = { ...formData, appointmentDateTime, status: "Scheduled" };

      await AxiosInstance.post("/lab-prescription", payload);
      setIsModalOpen(false);
      message.success("Lab Appointment Created & Scheduled Successfully!");
    ...
    <>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex flex-col gap-1">
          <h1 className="font-black text-slate-800 text-4xl tracking-tight">
            Lab Appointment <span className="text-blue-500">Creation</span>
          </h1>
          <p className="text-slate-500 font-medium">Resolve un-scheduled, missing, and unreviewed lab pipelines</p>
        </div>
      ...
      <div className="flex items-center gap-2 border-b border-slate-200 pb-4 mt-2">
        {["Not Scheduled", "Missing", "Not Reviewed"].map(tab => {
          ...
          const count = pendingLabs.filter(p => p.status === tab).length;
          return (
            <button
              key={tab}
            ...
            >
              {tab}
              <span className={`px-2 py-0.5 rounded-md text-[10px] ${isActive ? "bg-white/20" : "bg-slate-200"}`}>{count}</span>
            </button>
          )
        })}
      </div>
          ...
          <table className="w-full text-left border-separate border-spacing-0">
            <thead>
              <tr className="bg-slate-50 text-slate-500">
                <th className="px-6 py-5 font-black text-[10px] uppercase tracking-widest text-nowrap">Patient Details</th>
                <th className="px-6 py-5 font-black text-[10px] uppercase tracking-widest text-nowrap">Requested Lab</th>
                <th className="px-6 py-5 font-black text-[10px] uppercase tracking-widest text-nowrap">Added Date</th>
                <th className="px-6 py-5 font-black text-[10px] uppercase tracking-widest text-nowrap">Priority</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredData.length > 0 ? filteredData.map((row, i) => (
                <tr key={i} className="group hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 font-bold text-slate-500 text-sm">{dayjs(row.createdAt || row.appointmentDateTime).format("YYYY-MM-DD")}</td>
                  <td className="px-6 py-4">
                    <span className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider ${getPriorityColor(row.priority)}`}>
                      {row.priority}
                    </span>
                  </td>
                  <td className="px-6 py-4">
          ...
          <div className="bg-white/90 backdrop-blur-3xl rounded-3xl shadow-2xl border border-slate-200 w-full max-w-xl overflow-hidden">
            ...
            <form onSubmit={handleCreatePrescription} className="p-8 flex flex-col gap-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-[10px] font-black tracking-widest uppercase text-slate-500 pl-1">Scheduled Date</label>
                  <input type="date" required name="date" value={formData.date} onChange={handleInputChange} className="px-5 py-3 rounded-xl border border-slate-200 bg-slate-50/50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/40 text-sm font-bold text-slate-700 transition-all shadow-inner tracking-tight" />
                </div>
              </div>

              <div className="grid grid-cols-2 gap-6">
                <div className="flex flex-col gap-2">
                  <label className="text-[10px] font-black tracking-widest uppercase text-slate-500 pl-1">Scheduled Time</label>
                  <input type="time" required name="time" value={formData.time} onChange={handleInputChange} className="px-5 py-3 rounded-xl border border-slate-200 bg-slate-50/50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/40 text-sm font-bold text-slate-700 transition-all shadow-inner tracking-tight" />
                </div>
                <div className="flex flex-col gap-2">
        ...
        <div className="fixed inset-0 z-60 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md transition-all duration-300">
          <div className="bg-white/90 backdrop-blur-3xl rounded-3xl shadow-2xl border border-slate-200 w-full max-w-sm overflow-hidden p-6 scale-in">
            ...
            <div className="flex justify-end gap-3 mt-6 pt-6 border-t border-slate-100">
              <button onClick={() => { setSchedulingId(null); setScheduleDate({date: "", time: ""}); }} className="px-4 py-2.5 rounded-xl text-xs font-black text-slate-500 hover:bg-slate-100 uppercase tracking-widest transition-colors">Cancel</button>
...
export default LabAppointmentCreateForm;
