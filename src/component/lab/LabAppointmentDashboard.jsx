...
function LabAppointmentDashboard() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const res = await AxiosInstance.get('/lab-prescription/stats');
        ...
        <StaggerItem>
          <div className="flex flex-col gap-2 border-b border-slate-200 pb-6">
            <h1 className="font-black text-slate-800 text-4xl tracking-tight">
              Lab Appointment <span className="text-blue-500">Dashboard</span>
            </h1>
            <p className="text-slate-500 font-bold text-sm tracking-wide">Daily summary of lab center appointments and operational buckets</p>
          </div>
        </StaggerItem>
...
export default LabAppointmentDashboard;
